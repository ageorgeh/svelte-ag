...
export function useRovingFocus(props: UseRovingFocusProps) {
  ...
  function getCandidateNodes() {
    ...
    if (props.candidateSelector) {
      const candidates = Array.from(node.querySelectorAll<HTMLElement>(props.candidateSelector));
      return candidates;
    } else {
  ...
  function handleKeydown(node: HTMLElement | null | undefined, e: KeyboardEvent, both: boolean = false) {
