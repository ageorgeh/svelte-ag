<script lang="ts">
  ...
  import TasksContainer, { type NestedItem, type ContainerItem } from './TestContainer.svelte';
  import TaskItem from './TestItem.svelte';
...
<DndContext {sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragOver={handleDragOver}>
  ...
  <DragOverlay>
    ...
    {:else if isContainerItem(activeItem)}
      {@render tasksContainer(activeItem.data, activeItem.nesteds, 'shadow-xl')}
    {/if}
  </DragOverlay>
</DndContext>
...
{#snippet tasksContainer(data: NestedItem, nesteds: ContainerItem['nesteds'], className?: string)}
  <TasksContainer {data} type="container" accepts={['item']} class={className}>
    <SortableContext items={nesteds.map((item) => item.id)}>
      ...
      {:else}
        <p class="pt text-center text-sm font-medium text-gray-500">No tasks</p>
      {/each}
    </SortableContext>
  </TasksContainer>
{/snippet}
